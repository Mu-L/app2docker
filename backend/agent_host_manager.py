...
class AgentHostManager:
    ...
    def update_host_status(
        ...
        with self._lock:
            ...
            try:
                host_obj = (
                    db.query(AgentHost).filter(AgentHost.host_id == host_id).first()
                )
                if not host_obj:
                    return None
        ...
        with self._lock:
            ...
            try:
                ...
                if name and name != host_obj.name:
                    existing = (
                        db.query(AgentHost)
                        .filter(AgentHost.name == name, AgentHost.host_id != host_id)
                        .first()
                    )
                    if existing:
    ...
    def update_portainer_host_status(
        self, host_id: str, retry_count: int = 3
    ) -> Optional[Dict]:
        """
        ...
        try:
            ...
            for attempt in range(retry_count):
                try:
                    ...
                    if test_result.get("success"):
                        ...
                        if attempt > 0:
                            logger.info(
                                f"Portainer 主机 {host_obj.name} 在第 {attempt + 1} 次尝试后成功连接"
                            )
                        ...
                        if attempt < retry_count - 1:
                            ...
                            logger.debug(
                                f"Portainer 主机 {host_obj.name} 连接失败，{wait_time}秒后重试 ({attempt + 1}/{retry_count})"
                            )
                    ...
                    if attempt < retry_count - 1:
                        ...
                        time.sleep(wait_time)
                        logger.debug(
                            f"Portainer 主机 {host_obj.name} 连接异常，{wait_time}秒后重试 ({attempt + 1}/{retry_count}): {e}"
                        )
            ...
            if host_obj.status == "online":
                logger.warning(
                    f"Portainer 主机 {host_obj.name} 连接失败（{retry_count}次重试），标记为离线: {last_error}"
                )
    ...
    def generate_deploy_command(
        ...
        host_id: str,
        deploy_type: str = "run",
        agent_image: str = "registry.cn-shanghai.aliyuncs.com/51jbm/app2docker-agent:latest",
